  ...
  try {
    ...
    if (error) {
      if (error.code === 11000) {
        ...
        res.json({
          message: error.message || "error on signup please try again",
        });
      }
    }
  }
};
export const otpCreation = async (
  req: Request,
  res: Response,
  authService: AuthService,
...
export const login = async (
  ...
  try {
    ...
    const { token, refresh, name, photo, partner, gender, subscriptionStatus } =
      response;
...
export const fetechProfileData = async (
  req: Request,
  res: Response,
  partnersProfileService: PartnerProfileService
) => {
  try {
    if (req.userID) {
      const response = await partnersProfileService.fetchProfileData(
        req.userID,
        req.gender,
        req.preferedGender
      );
...
export const forgotCheckValidate = async (
  req: Request,
  res: Response,
  otpService: OtpService
): Promise<void> => {
  try {
    if (typeof req.query.email === "string") {
      ...
      if (isValid) {
        const response = await otpService.otpVerificationForForgot(
          email,
          "forgot"
        );
        if (response) {
...
export const forgotCheckValidateSigunp = async (
  req: Request,
  res: Response,
  otpService: OtpService
): Promise<void> => {
...
export const secondBatch = async (
  req: Request,
  res: Response,
  ...
  } = { responseFromAddinInterest: false, url: false };
  try {
    if (req.file && req.file.path) {
      const image = req.file?.path || "";

      const response =
        (await userProfileService.uploadPhoto(image, req.body.email)) || "";

      obj.url = response;
    }
    const interest: string[] = JSON.parse(req.body.interest);
    if (interest.length > 0) {
      const responseFromAddinInterest = await userProfileService.uploadInterest(
        interest,
        req.body.email
      );
...
export const addMatch = async (
  req: Request,
  res: Response,
  patnerServiece: PartnerProfileService
): Promise<void> => {
  try {
    const response = await patnerServiece.addMatch(
      req.userID,
      req.body.matchId
    );
...
export const manageReqRes = async (
  req: Request,
  res: Response,
  partnersProfileService: PartnerProfileService
): Promise<void> => {
  try {
    const response = await partnersProfileService.manageReqRes(
      req.body.id,
      req.userID,
      req.body.action
    );
...
export const purchasePlan = async (
  req: Request,
  res: Response,
  paymentSerivice: PaymentSerivice
): Promise<void> => {
  try {
    if (!req.userID) {
    ...
    if (req.body.planData && req.body.token && req.body.token.email) {
      const response = await paymentSerivice.purchase(
        req.body.planData,
        req.body.token,
...
export const fetchDataForProfile = async (
  req: Request,
  res: Response,
  partnerServiece: PartnerProfileService
): Promise<void> => {
  try {
...
export const fetchInterest = async (
  req: Request,
  res: Response,
  interestService: FixedDataService
): Promise<void> => {
  try {
...
export const otpForResetPassword = async (
  req: Request,
  res: Response,
  otpService: OtpService
) => {
...
export const otpForUserResetPassword = async (
  req: Request,
  res: Response,
  otpService: OtpService
) => {
  try {
    const validate = await otpService.validateOtpForEditProfiel(
      req.userID,
...
export const resetPassword = async (
  ...
  try {
    ...
    if (password === confirmPassword) {
      const response = await authService.changePasswordEditProfile(
        password,
        req.userID
      );
...
export const editProfile = async (
  ...
  try {
    if (req.file) {
      ...
      const updateDetail = await userProfileService.updateEditedData(
...
export const matchedUser = async (
  req: Request,
  res: Response,
  partnerServiece: PartnerProfileService
) => {
...
export const deleteMatched = async (
  req: Request,
  res: Response,
  partnerServiece: PartnerProfileService
) => {
...
export const fetchSuggestion = async (
  ...
  try {
    const result = await partnerServiece.fetchSuggestions(
      req.userID,
      req.preferedGender,
      req.gender
    );
...
export const createTexts = async (
  req: Request,
  res: Response,
  chatService: ChatService
) => {
...
export const MsgCount = async (
  ...
  try {
    const newMessagesForNav = await messageService.fetchMessageCount(
      req.query?.from,
      req.userID
    );
    const newMessagesNotifiation = await messageService.findNewMessages(
      req.userID
    );
...
export const MessageViewed = async (
  req: Request,
  res: Response,
  messageRepo: MessageService
) => {
  try {
    const response = await messageRepo.makeAllUsersMessageReaded(
      req.body.from,
      req.body.ids
    );
    res.json({ status: response });
...
export const planHistoryAndRequest = async (
  req: Request,
  res: Response,
