  ...
  implements IOtherRepositories
{
  ...
  async isExist() {
    try {
      const exitstingData: IInterestInterface | null =
...
export class FeaturesRepository
  extends BaseRepository<IFeatures>
  implements IFeaturesRepository
{
  ...
  async fetchFeature() {
    try {
      const response: { features: IFeatures } | null =
        await featureModel.findOne({}, { _id: 0, features: 1 });
      return response;
    } catch (error) {
      if (error instanceof Error) {
        throw new Error(error.message);
...
export class TokenRepository
  extends BaseRepository<IRefeshToken>
  ...
  async fetchToken(
    ...
    try {
      return await this.model
        .findOne({ token: refreshToken, userId: extractId })
        .populate<IRefreshWithPopulatedData>("userId");
    } catch (error) {
      if (error instanceof Error) {
        throw new Error(error.message);
      } else {
