...
export class PurchasedPlanRepository extends BaseRepository<PlanOrderMongo> implements IPurchasedPlan {
  constructor(){
  ...
  async fetchHistory(id:string){
    try {
      const data:IPlanOrder[]=await this.model.aggregate([{$match:{userID:new Types.ObjectId(id)}},{$sort:{_id:-1}},{$skip:1}])
        return data
    } catch (error) {
      if (error instanceof Error) {
        throw new Error(error.message);
