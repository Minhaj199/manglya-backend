...
export const firstBatchDataDtoSchema=z.object({
    ...
    'SECOND NAME':z.string({ required_error: "second name required" }).min(1,'second name shoud be more thant ').max(10,'first name shoud be less thant 10'),
      ...
      'EMAIL':z.string({ required_error: ResponseMessage.EMAIL_REQUIRED}).email(ResponseMessage.VALID_EMAIL),
...
export const loginDtoSchema=z.object({
  email:string({required_error:ResponseMessage.EMAIL_REQUIRED}).email(ResponseMessage.VALID_EMAIL),
  password:string({required_error:'Password required'}).min(3,'enter a valid passoword')
})
...
export const otpCreationDtoSchema=z.object({
  email:string({required_error:ResponseMessage.EMAIL_REQUIRED}).email(ResponseMessage.VALID_EMAIL),
  from:string({required_error:ResponseMessage.FROM_ARGUEMENT_MISSING}).min(3,ResponseMessage.FROM_ARGUEMENT_MISSING)
})
...
export const otpDtoSchema=z.object({
  email:string({required_error:ResponseMessage.EMAIL_REQUIRED}).email(ResponseMessage.VALID_EMAIL),
  from:string({required_error:ResponseMessage.FROM_ARGUEMENT_MISSING}).min(3,ResponseMessage.FROM_ARGUEMENT_MISSING),
  otp:string({required_error:'otp requrired'}).min(6,'number should be 6 digits').max(6,'number should be 6 digits')
})
...
export const passwordResetDTOSchema=z.object({
  email:string({required_error:ResponseMessage.EMAIL_REQUIRED}).email(ResponseMessage.VALID_EMAIL),
  password:string({required_error:'password required'}).refine((value)=>passwordRegex.test(value),{
    message:'passoword not strong please add Upper,lower cases,number and special charetors',
    path: ["password"]
  }),
...
export const secondBatchDtoSchema=z.object({
  email:string({required_error:ResponseMessage.EMAIL_REQUIRED}).email(ResponseMessage.VALID_EMAIL),
  interest:z.string({required_error:'interest required'}).refine((val:string)=>{
   try {
      ...
      if (!Array.isArray(parserInterestArray)) {
        ...
        return false
      }else if(parserInterestArray.some(item => typeof item !== 'string')){
        return false
      }
...
export const acssTknRenualDTOSchema=z.object({
  refresh:string({required_error:'refresh token required'}).min(3,'valid refresh token missing'),
...
export const createTextsDTOSchema=z.object({
        chatId:string({required_error:'chat id required'}).min(3,'not valid chatID'),
        senderIdString:string({required_error:'sender id required'}).min(3,'not valid sender id'),
        receiverId:string({required_error:'reciever required'}).min(3,'not valid reciever id'),
...
export const reportAbuserUserDTOSchema=z.object({
        profileId:string({required_error:'profile id required'}).min(3,'enter a valid id'),
        reason:string({required_error:'reason required'}).min(3,'inter a valid reason'),
        moreInfo :string({required_error:'more info required'}).min(3,'provide more info')
})
...
export const addMatchDtoSchema=z.object({
        matchId:string({required_error:'mached id required'}).min(3,'valid match id required'),
})

export const acceptAndRejectDTOSchema=z.object({
        id:string({required_error:'requester id required'}).min(3,'valid requester id required'),
        action:string({required_error:'action required'}).min(3,'valid action required')
})
...
export const deleteMatchedUser=z.object({
  id:array(string(({required_error:ResponseMessage.ID_NOT_FOUND})).min(1,'id not foudn')),
...
export const udapteUserProfileDTOSchema=z.object({
  ...
  PersonalInfo:z.object({
    firstName:string({required_error:'first name string type expected'}).optional(),
    secondName: string({required_error:'last name string type expected'}).optional(),
    state: string({required_error:'state string type expected'}).optional(),
    gender: string({required_error:'gender string type expected'}).optional(),
    dateOfBirth: date({required_error:'date of birth date type expected'}).optional(),
    image: string({required_error:'image string type expected'}).optional(),
    interest: array(string({required_error:'interest string type expected'})).optional(),
    photo: string({required_error:'photo string type expected'}).optional()
  }),
  partnerData:z.object({ gender: string().optional()}),
  email: string().email(),
  subscriber: string({required_error:'subscriber string type expected'}).optional()
