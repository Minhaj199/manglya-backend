...
export class ParternDataChatList implements IParternDataChatListDTO {
  ...
  onlines: string[];
  constructor(
    private rowParternData: Omit<IExtentedMatchProfile, "lastName">[],
    private rowPlaces: string[],
    private rowOnliners: string[]
  ) {
    this.connectedParterns =
      this.rowParternData.map((elem) => {
        return {
          _id: elem._id,
          state: elem.state,
          photo: elem.photo,
