...
import { IAdminPlanType, SubscriptionPlanDocument } from "../types/TypesAndInterfaces";
...
export class PlanDTO implements IPlanDTO{
   ...
   constructor (planData:SubscriptionPlanDocument[]|[]){
        this.plans=planData?.map((elem:SubscriptionPlanDocument)=>{
            return{
                _id:elem._id,
                name:elem.name,
...
export class SubscriberPlanDTO {
    ...
    constructor(private plan:{name:string}[],private userDataDraft: IAdminPlanType[]){
        this.planData= plan?.map((el) => {
        return { name: el.name };
      })||[]
      this.userData=userDataDraft?.map((el, index) => {
          return {
            no: index + 1,
